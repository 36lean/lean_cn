...
class table_b_video extends discuz_table {
	...
	public function insert_for_page( $data) {
		return DB::query('insert into '.DB::table($this->_table).'(v_file,v_path,label_cn,label_tw,label_en,image_file,v_voice) values('.$data['v_file'].','.$data['v_path'].','.$data['label_cn'].','.$data['label_tw'].',' . $data['label_en'] . ','.$data['image_file'].',0,1,1)');
	}
	...
	public function update_video( $data) {
		DB::query('update '.DB::table( $this->_table).' set v_file = '.$data['v_file'].',v_path = '.$data['v_path'].',label_cn = '.$data['label_cn'].',label_tw = '.$data['label_tw'].',label_en = '.$data['label_en'].',cn_intro = '.$data['cn_intro'].',en_intro = '.$data['en_intro'].',image_file = '.$data['image_file'].',updated_date = '.time().',v_time = '.$data['v_time'].',v_voice = '.$data['v_voice'].' where id = '.$data['film_id']);
	}
	...
	public function get_video_info_by_id( $id) {
		return DB::fetch_first( 'select v.v_path,v.label_cn,v.label_tw,v.label_en,v.cn_intro,v.en_intro,v.image_file,v.v_time,v.v_voice,v.v_name from '.DB::table( $this->_table) .' as v left join '.DB::table( $this->_jointable).' as p on p.film_id = v.id where p.id = '.$id);
	}
