...
class table_b_course extends discuz_table {
	...
	public function get_all_course() {
		return DB::fetch_all('select id,fullname from '.DB::table($this->_table).' where is_hidden = %d' , array(0));
	}
	...
	public function count_of_course() {
		return DB::fetch_first('select count(id) as sum from '.DB::table( $this->_table).' where is_hidden = 0');
	}
}