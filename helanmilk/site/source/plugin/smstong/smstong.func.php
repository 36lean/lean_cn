...
function sendsms($user, $pass, $mobile, $content, $checkmobile=true, $refno='', $creditchange=true)
{
	if(empty($_SESSION['smstong_mobile']) || $mobile != $_SESSION['smstong_mobile']) {
		...
		if($checkmobile) {
			...
			$count2 = DB::result_first("SELECT count(id) FROM sms_send WHERE mobile='$mobile' AND addtime>'$_G[timestamp]'-86400");
		...
		if($ret === true) {
			...
			if ($_G['cache']['plugin']['smstong']['sendtype'] == "1") {
				...
				$app_id = "101196700000029879";
				$account = $_G['cache']['plugin']['smstong']['surfingmobile'];
				$app_secret = $_G['cache']['plugin']['smstong']['app_secret'];
				$access_token = $_G['cache']['plugin']['smstong']['accesstoken'];
				$timestamp = date('YmdHis', time());

				$message = '';

				if ($_G['charset'] == "gbk")
					$message = iconv('GBK', 'UTF-8//IGNORE', $content);
				else
					$message = $content;
				...
				$data = array(
					...
					"dest" => $mobile,
					"message" => $message,
					"sign" => md5($app_id.$access_token.$timestamp.$app_secret),
					"app_id" => $app_id,
					"access_token" => $access_token,
					"timestamp" => $timestamp
				);

				//$sendurl = "http://api.189.cn/ecp/sms/sendsms?account=".$account."&dest=".$mobile."&message=".urlencode(iconv('GBK', 'UTF-8//IGNORE', $content))."&sign=".md5($app_id.$access_token.$timestamp.$app_secret)."&app_id=".$app_id."&access_token=".$access_token."&timestamp=".$timestamp."";

				$ret = httprequest("http://api.189.cn/ecp/sms/sendsms", $data);

				if ($ret && stristr($ret, '"code":"1100"')) {
					...
					foreach($mobiles as $k => $v) {
						if(empty($refno)) {
							DB::query("INSERT INTO sms_send (mobile,content,addtime,senttime,count,status,port) values('$v', '$content', '$addtime', '$addtime', $count, 1, 'telecom')");
						}
						else if(strpos($refno, ',')) {
							DB::query("INSERT INTO sms_send (mobile,content,addtime,senttime,count,status,port,remark) values('$v', '$content', '$addtime', '$addtime', $count, 1, 'telecom', '$refno')");
						} else {
							DB::query("INSERT INTO sms_send (mobile,content,addtime,senttime,count,status,port,refno) values('$v', '$content', '$addtime', '$addtime', $count, 1, 'telecom', $refno)");
						}
					}
			...
			} elseif ($_G['cache']['plugin']['smstong']['sendtype'] == "2") {
