...
function showsendsms() {
	...
	showtablerow('', array('class="th12"', ''), array(
		...
		'<a href="'.ADMINSCRIPT.'?action=plugins&operation=config&do='.$pluginid.'&identifier=smstong&pmod=qunfasms&getmobile=true">'.$Plang['smstong_members_qunfasms_getmobile'].'</a><br /><br /><textarea name="message" id="message" cols="100" rows="10" onkeyup="javascript:checkNum();">'.$_G['gp_content'].'</textarea> '.$Plang['smstong_sendsms_checkwords'].'<script language="javascript">checkNum();function checkNum(){ var content=document.getElementById("message").value + "'.$sign.'"; document.getElementById("showid").innerText=content.length; document.getElementById("sign").innerText= "'.$sign.'";}</script>'
	));
