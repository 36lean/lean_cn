...
<!--{if $_G['setting']['search'] && $slist}-->
	...
	<form id="scbar_form" method="{if $_G[fid] && !empty($searchparams[url])}get{else}post{/if}" autocomplete="off" onsubmit="searchFocus($('scbar_txt'))" action="{if $_G[fid] && !empty($searchparams[url])}$searchparams[url]{else}search.php?searchsubmit=yes{/if}" target="_blank">
		...
		<!--{/if}-->
			...
			<a href="javascript:;" id="scbar_type" class="showmenu xg1" onclick="showMenuu(this.id)" hidefocus="true">{lang search}</a>



			<input type="text" name="srchtxt" class="ipt" id="scbar_txt" value="{lang enter_content}" autocomplete="off" x-webkit-speech speech />



			<button type="submit" name="searchsubmit" class="btn" value="true"></button>
...
<ul id="scbar_type_menu" class="p_pop" style="display: none; margin-top:0px; width:40px;"><!--{echo implode('', $slist);}--></ul>
