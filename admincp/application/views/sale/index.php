		...
		$.ajax( {
			...
			success : function( data) {
				...
				}else
				{
					...
					$('.page-header').append( '客户<strong>'+data.name+'</strong>的沟通记录添加成功!');					
				}
...
<table class="mot-table">
	...
	<tr>
		...
		<td class="switch_color <?php echo $color[$one['display_color']];?>">
		...
		<td>
			...
			<select class="mot-select" data-source="<?php echo $one['id'];?>">
				<?php foreach ($tags as $tag) { ?>
					<option value="<?php echo $tag['id'];?>" <?php if($tag['id'] == $one['tag']){?>seleted="selected"<?php }?>><?php echo $tag['tag'];?></option>
				<?php }?>	
			</select>
		...
		<td>
			<a href="<?php echo site_url('marketing/connect/'.$one['id']);?>" id="<?php echo $one['id'];?>"><?php echo $one['name'];?></a>
		</td>
...
<script src="<?php echo base_url('public/mot/js/call.js');?>"></script>
<script src="<?php echo base_url('public/mot/js/contacts_fixed.js');?>"></script>
